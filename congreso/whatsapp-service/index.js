...
async function connectToWhatsApp() {
  try {
    ...
    sock = makeWASocket({
      ...
      auth: state,
      browser: ['Congreso María Camino a Jesús', 'Chrome', '1.0.0'],
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        console.log(`✅ ¡WhatsApp Conectado con éxito! Número: ${sock.user?.id || 'MCJ'}`);
      }
...
// Visual Web Dashboard to scan QR easily from browser/mobile
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="es">
    <head>
      ...
      <title>Bot de WhatsApp • Congreso María Camino a Jesús</title>
    ...
    <body class="bg-slate-950 text-slate-100 min-h-screen flex items-center justify-center p-4">
      <div class="max-w-md w-full rounded-3xl border border-amber-500/30 bg-slate-900/90 p-6 text-center shadow-2xl space-y-5">
        <div class="flex items-center justify-center gap-2 text-amber-400 font-bold text-sm">
          <span>✨ Congreso María Camino a Jesús</span>
        </div>
        ...
        ${isConnected ? `
          <div class="rounded-2xl bg-emerald-950/80 border border-emerald-500/40 p-5 text-center space-y-2">
            ...
            <p class="text-xs text-slate-300">Vinculado a: <strong>${connectedUser?.name || connectedUser?.id?.split(':')[0] || 'Asociación MCJ'}</strong></p>
            <p class="text-[11px] text-slate-400 pt-2">El bot está listo para despachar entradas y pases con QR.</p>
          </div>
        ...
        <div class="pt-3 border-t border-slate-800 text-[10px] text-slate-500">
          Asociación Pública de Fieles María Camino a Jesús • Puerto ${PORT}
        </div>
      </div>
      <script>
